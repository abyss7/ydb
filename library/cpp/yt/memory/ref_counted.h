...
#define DECLARE_REFCOUNTED_TYPE(type) \
    ...
    \
    [[maybe_unused]] YT_ATTRIBUTE_USED Y_PUBLIC const ::NYT::TRefCounter* GetRefCounter(const type* obj); \
    [[maybe_unused]] YT_ATTRIBUTE_USED Y_PUBLIC void DestroyRefCounted(const type* obj); \
    [[maybe_unused]] YT_ATTRIBUTE_USED Y_PUBLIC void DeallocateRefCounted(const type* obj);
...
//! Y_PUBLIC: the out-of-line copy forced by YT_ATTRIBUTE_USED must stay exported from
//! a shared library under -fvisibility-inlines-hidden, since users of |public.h| only
//! see the declaration.
#define DEFINE_REFCOUNTED_TYPE(type) \
    [[maybe_unused]] YT_ATTRIBUTE_USED Y_PUBLIC Y_FORCE_INLINE const ::NYT::TRefCounter* GetRefCounter(const type* obj) \
    { \
        return ::NYT::NDetail::TRefCountedTraits<type>::GetRefCounter(obj); \
    } \
    [[maybe_unused]] YT_ATTRIBUTE_USED Y_PUBLIC Y_FORCE_INLINE void DestroyRefCounted(const type* obj) \
    { \
        ::NYT::NDetail::TRefCountedTraits<type>::Destroy(obj); \
    } \
    [[maybe_unused]] YT_ATTRIBUTE_USED Y_PUBLIC Y_FORCE_INLINE void DeallocateRefCounted(const type* obj) \
    { \
